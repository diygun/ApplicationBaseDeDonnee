...
namespace Projet_DB_ManagmentAPP.Acces
{
    ...
    public class A_t_produit : ADBase
    {
        ...
        public int Ajouter(string Nom, decimal Prix_vente, decimal Prix_achat, int Quantite_stock, double TVA, int Seuil_stock, DateTime? DateSortie)
        {
            ...
            if (DateSortie == null) Commande.Parameters.AddWithValue("@DateSortie", Convert.DBNull);
            else Commande.Parameters.AddWithValue("@DateSortie", DateSortie);
        ...
        public List<C_t_produit> Lire(string Index)
        {
            ...
            while (dr.Read())
            {
                ...
                if (dr["DateSortie"] != DBNull.Value) tmp.DateSortie = DateTime.Parse(dr["DateSortie"].ToString());
                res.Add(tmp);
            }
            dr.Close();
            Commande.Connection.Close();
        ...
        public C_t_produit Lire_ID(int ID_produit)
        {
            ...
            while (dr.Read())
            {
                ...
                if (dr["DateSortie"] != DBNull.Value) res.DateSortie = DateTime.Parse(dr["DateSortie"].ToString());
